...
class TestSaveMemory:
    def test_executes_insert_and_delete(self) -> None:
        conn_mock = make_conn_mock()
        with patch("agent.memory.manager.get_connection", return_value=conn_mock), \
             patch("agent.memory.manager.embed", return_value=DUMMY_EMBEDDING), \
             patch("agent.memory.manager.release_connection") as mock_release:
            save_memory("session-1", "テスト内容")

        cursor_mock = conn_mock.cursor.return_value
        assert cursor_mock.execute.call_count == 2
        insert_sql = cursor_mock.execute.call_args_list[0][0][0]
        delete_sql = cursor_mock.execute.call_args_list[1][0][0]
    ...
    def test_embedding_is_passed(self) -> None:
        conn_mock = make_conn_mock()
        with patch("agent.memory.manager.get_connection", return_value=conn_mock), \
             patch("agent.memory.manager.embed", return_value=DUMMY_EMBEDDING) as mock_embed, \
             patch("agent.memory.manager.release_connection"):
            save_memory("session-1", "テスト内容")

        mock_embed.assert_called_once_with("テスト内容")
        params = conn_mock.cursor.return_value.execute.call_args_list[0][0][1]
