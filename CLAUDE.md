...
- ユーザーから「タスクを振って」と指示された場合:
  1. TODO.md を読み込み、未着手(`[ ]`)のタスクを確認する
  2. 依存関係（ブロック）が解消済みのタスクから優先度順に提案する
  3. タスクの概要・受入条件を提示し、ユーザーの承認後に作業を開始する
  4. タスク着手時に TODO.md のステータスを `[>]` に更新する
  5. タスク完了時に TODO.md のステータスを `[x]` に更新する
...
| チケット管理 | TODO.md | 実装タスク一覧・進捗管理（PMが管理） |
