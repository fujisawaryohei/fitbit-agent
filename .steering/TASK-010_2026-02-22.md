...
## 受入条件
- [x] `GET /api/auth/login` でFitbit認可URLにリダイレクトされる
- [x] `GET /api/auth/callback` でトークン取得・暗号化保存・セッション生成が行われる
- [x] トークン自動リフレッシュが動作する
...
- [x] Step 2: User エンティティ作成
- [x] Step 3: OAuthToken エンティティ作成
- [x] Step 4: UserRepository / OAuthTokenRepository 作成
- [x] Step 5: TokenEncryptionService 作成（AES-256-GCM）
- [x] Step 6: FitbitConfig 作成
- [x] Step 7: application-dev.yml に Fitbit OAuth2 設定追加
- [x] Step 8: SecurityConfig 作成
- [x] Step 9: FitbitOAuthClient 作成（トークン交換・リフレッシュ）
- [x] Step 10: AuthService 作成
- [x] Step 11: AuthController 作成
- [x] Step 12: WebConfig 作成（CORS設定）
- [x] Step 13: GlobalExceptionHandler 作成
- [x] Step 14: 動作確認
...
**WebConfig の作成も必要**。`SecurityConfig.java` に追加した `WebClient` の Bean を `WebConfig.java` に移動する。
...
**ファイルパス**: `backend/src/main/java/com/fitbitagent/exception/GlobalExceptionHandler.java`

Controller で例外が発生した場合に、適切な HTTP レスポンスを返す。
`@RestControllerAdvice` を使い、例外の種類ごとにハンドラーメソッドを定義する。

### 対象とする例外

| 例外クラス | HTTPステータス | 意味 |
|---|---|---|
| `FitbitApiException` | 502 Bad Gateway | Fitbit API との通信失敗 |
| `RuntimeException` | 500 Internal Server Error | 予期しないエラー全般 |

### 実装方針

- `@RestControllerAdvice` — Controller の例外をグローバルにキャッチするアノテーション
- `@ExceptionHandler(XxxException.class)` — キャッチする例外の型を指定
- レスポンスボディはシンプルな `Map<String, String>` で `{"error": "メッセージ"}` の形式
- `@Slf4j` でログ出力を忘れずに
...
### ポイント
- `FitbitApiException` を先に、`RuntimeException` を後に定義する
  （`FitbitApiException extends RuntimeException` のため、順序が逆だと全部 500 になってしまう）
- エラーメッセージに内部情報を含めないよう、`RuntimeException` は固定文言を返す
